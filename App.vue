<script>
	...
	export default {
		globalData: {
			...
			agent_id: 1,
			/** http 服务端地址 */
			http_url: 'http://ima.dragonloo.co',
			/** 静态文件存放地址 */
			// new_static_url: "http://looimoss.oss-cn-hongkong.aliyuncs.com",
			static_url: 'http://ima.dragonloo.co',
			/** socket 服务端地址 */
			socket_url: 'ws://imws.dragonloo.co',
			// socket_url: 'wss://xxx.xxx.com/wss',
			/** socket 连接状态 */
			socket_state: 0,
			/** 好友申请通知 */
		...
		onLaunch() {
			...
			plus.globalEvent.addEventListener('FileChooseResult', (e) => {
				...
				var filetype = 10;
				//文件大小
				if (e.fileType == "jpg" || e.fileType == "jpeg" || e.fileType == "gif" || e.fileType == "png" || e
					.fileType == "bmp" || e.fileType == "webp" || e.fileType == "pjpej") {
					filetype = 2;
