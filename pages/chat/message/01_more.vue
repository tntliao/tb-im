	...
	<view class="page">
		...
		<view class="photos" v-if="data.type == 0">
			<view class="photo_main" v-for="(val,index) of data.member" :key="index" @tap="goDetails(val.user_id)">
				<image class="photo" :src="userPhoto(val.photo+'')" :lazy-load="true" style="border-radius: 10px;"
		...
		<view class="uni-card" v-if="data.type == 1">
			<view class="uni-list">
				<view class="list-member-count">
					<view class="member_count" v-if="data.type == 1" style="padding: 0 24upx;">
					...
					<view class="list-footer" v-if="data.type == 1">
						...
						<view class="member-list">
							<view class="member-item" v-for="(val,index) of data.member" :key="index"
								@tap="goDetails(val.user_id)">
								<view class="item-left">
									<view class="item-left-img">
										<image :src="userPhoto(val.photo+'')" :lazy-load="true"
											style="border-radius: 10px;" :data-index="index" @erro="imageError"></image>
									</view>
								...
								<view class="item-right">
									<view class="item-right-img">
										<image src="/static/theme/default/qunzhu.png" v-show="val.is_admin == 2">
										</image>
										<image src="/static/theme/default/guanliyuan.png" v-show="val.is_admin == 1">
										</image>
									</view>
					...
					<view class="uni-list-cell" @tap="goSetGroupPhoto">
						<view class="uni-media-list uni-list-cell-navigate uni-navigate-right">
							...
							<view class="uni-media-list-logo photo">
								<image :src="photo(data.group.is_photo)" :lazy-load="true"
					...
					<view class="uni-list-cell" @tap="groupCopy">
						<view class="uni-media-list uni-list-cell-navigate">
							...
							<view class="uni-media-list-logo qun-id">
								<view class="show_text msg-size" style="overflow: hidden;">
									{{replaceStr(data.group.id + '')}}
								</view>
							</view>
						</view>
					</view>
				...
				<view class="list-member-count">
					...
					<!-- <view class="uni-list-cell" @tap="checkfile()">
						<view class="uni-list-cell-navigate uni-navigate-right">
							<view class="qun-gonggao">
								<text  class="msg-size">群文件</text>
								<view class="show_text_ msg-size"></view>
							</view>
						</view>
					</view> -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			userPhoto(path) {
				return this.getNewAvatarPath('/photo/', path) + "&x-oss-process=image/resize,m_fill,w_50,quality,q_60";
				// return getApp().globalData.new_static_url + '/photo/' + path + '?_=' + Math.random();
			},
			msgChange(e) {
				let value = e.detail.value ? 1 : 0;
			...
			checkfile() {
				uni.navigateTo({
					url: '../message/file_list?list_id=' + this.list_id
				})
			},
			xiaoHui() { //销毁聊天记录
				_get.xiaoHui({
