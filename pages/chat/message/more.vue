							...
							<view class="member-item" v-for="(val,index) of data.member" :key="index"
								...
								<view class="item-left">
									<view class="item-left-img">
										<image :src="photo(val.photo+'')" :lazy-load="true" style="border-radius: 10px;"
											:data-index="index" @erro="imageError"></image>
									</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			photo(path) {
				const url = this.staticPhoto + path
				return url.replace("/300.jpg", "/50.jpg");
			},
			msgChange(e) {
				let value = e.detail.value ? 1 : 0;
