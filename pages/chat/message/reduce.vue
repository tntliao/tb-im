<template>
	<view class="page">
		<scroll-view class="scrollList" scroll-y>
			<view class="search">
				<uni-search-bar ref="searchBar" placeholder="搜索好友" @confirm="search" @search="search">
				</uni-search-bar>
			</view>
			<view class="uni-list">
		...
		methods: {
			search(value) {
				...
				this.timer = setTimeout(() => {
					const arr = [];
					this.copy_list_data.forEach(item => {
						console.log(item, '11');
						const index = item.nickname.indexOf(value);
						const indexs = item.username.indexOf(value);
						if (index != -1 || indexs != -1) {
							arr.push(item);
						}
					})
					this.list_data = [];
					this.list_data.push(...arr)
				}, 700)
