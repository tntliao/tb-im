<template>
	<view>
		...
		<view class="info">
			已领取{{hong_bao_detail.number - hong_bao_detail.sy_number}}/{{hong_bao_detail.number }}个，共{{(hong_bao_detail.money - hong_bao_detail.sy_money).toFixed(2)}}/{{hong_bao_detail.money}}
			USDT
		</view>
		<view class="list">
			<view class="row" v-for="(row,index) in list" :key="index">
				<view class="left">
					<image :src="getNewAvatar(staticPhoto,row.user_info.face)"></image>
				</view>
				<view class="right">
					<view class="r1">
						<view class="username">
